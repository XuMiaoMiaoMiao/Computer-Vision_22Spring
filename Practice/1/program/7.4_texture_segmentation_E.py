...
def textures_segmentation_entropy(image):
    '''
    ...
    #Calculate the entropy of the picture
    E = filters.rank.entropy(Igray,kernel).astype(np.float32)
    #Normalize the obtained float32 value 
    #and convert it to the range of 0~255
    Eim = (E-E.min()) / (E.max() - E.min())
    Eim = np.uint8(Eim*255)
    #Binarize the image using 
    #the Otsu thresholding method
    ret,BW1 = cv2.threshold(Eim,0,255,cv2.THRESH_OTSU)
...
segmentedFrames1 = textures_segmentation_entropy(I1)
...
plt.suptitle('histogram symmetry characteristic: using entropy')
plt.show()
